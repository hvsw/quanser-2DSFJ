...
// -----------------------------------------------------------------------------
void clear_counter() {
    uint32_t buf_escrita_32 = 0x20000000;
...
int openFiles() {
    ...
    // DIRECTION
    if ((_fd.direction = open(DIRECTION, O_WRONLY)) < 0) {
...
void testPins(int numIterations, int interval) {
    ...
    while(count < numIterations) {
        ...
        // DIRECTION
        if (writeGPIO(_fd.direction, count%2) < 0) {
            fprintf(stderr, "%s - Error writing to DIRECTION\n", __FUNCTION__);
        }
        // EN
